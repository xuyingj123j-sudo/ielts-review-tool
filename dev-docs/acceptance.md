...
- 提示条：CDP 真鼠标进入单卡，覆盖 flip/spelling；自由练习覆盖拼写提交、下一张、翻卡与完成页，均断言准确文案和可见性；正式复习无提示条。自测前后卡片逐行不变，`review_logs=0`。
- 任务模板：真实 curl 覆盖默认三项、新增、改名、勾选/取消和软删除。自定义任务今天及昨天完成时复盘 `2/7`，改名后历史显示新名，删除今天后历史仍为 `1/7`。全删后当天/次日均为空，重启迁移不补种；同名任务按日期去重、特殊名称及错误输入有断言。
- 迁移：真实数据库只读备份后在临时副本执行。39张卡片、22条旧任务、写作、听力与数字记录逐行保持；`integrity_check=ok`，`foreign_key_check=[]`。正式数据库未迁移或写入。
- 真题隐藏：CDP 首页、导航及其他可达页面没有听力真题入口；旧导航目标回首页。副本 `curl GET /api/listening/overview` 返回200，剑16 Test1四个Part及音频路径均在；`test_listening.js` 全绿，音频路由200。后端听力代码、表定义及音频未修改。
- 移动端：六张反馈页面截图均为 `width=scroll=body=390`；人工复核警示色、圆角输入框、动态任务统计及首页入口。数字专项旧测试的390/320布局、分类颜色与真实鼠标播放后输入仍通过。

未部署、未操作PM2、未推送GitHub、未连接线上服务器。测试服务器及独立Chrome均随测试关闭，临时数据库/profile自动清理。截图和日志目录 `C:\Users\xuyin\AppData\Local\Temp\ielts-feedback-evidence-7dfd7c8fc470446ebe7329b274cba07b` 的递归删除和仅删除已核对文件均被环境自动审批拒绝（blocked by policy，未提供细分原因）；目录仍残留，临时产物清理尚未完成。
